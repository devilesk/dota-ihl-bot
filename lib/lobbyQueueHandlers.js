...
const LobbyQueueHandlers = ({ Db, Lobby }) => ({
    async [CONSTANTS.QUEUE_TYPE_DRAFT](_lobbyState) {
        ...
        logger.silly(`QUEUE_TYPE_DRAFT ${queuers.length} ${lobbyState.id} ${lobbyState.lobbyName}`);
    ...
    async [CONSTANTS.QUEUE_TYPE_AUTO](_lobbyState) {
        ...
        logger.silly(`QUEUE_TYPE_AUTO ${queuers.length} ${lobbyState.id} ${lobbyState.lobbyName}`);
        if (queuers.length >= 10) {
            lobbyState.state = CONSTANTS.STATE_BEGIN_READY;
    ...
    async [CONSTANTS.QUEUE_TYPE_CHALLENGE](_lobbyState) {
        const lobbyState = { ..._lobbyState };
        logger.silly(`LobbyQueueHandlers QUEUE_TYPE_CHALLENGE ${lobbyState.id} ${lobbyState.lobbyName} ${lobbyState.captain1UserId} ${lobbyState.captain2UserId}`);
