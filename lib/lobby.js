...
const checkQueueForCaptains = getUserRoles => async lobbyState => pipeP(
    ...
    selectCaptainPairFromTiers(lobbyState.inhouseState.captain_rank_threshold),
)(lobbyState);
...
const lobbyToLobbyState = ({ findOrCreateChannelInCategory, makeRole }) => inhouseState => async (lobby) => {
    let channel;
    let role;
    try {
...
const runLobby = async _lobbyState => lock.acquire(`runLobby-${_lobbyState.id}`, async () => {
    ...
    let lobbyState = await pipeP(
        createLobbyState(_lobbyState.inhouseState)(_lobbyState),
