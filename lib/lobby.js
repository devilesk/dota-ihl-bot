...
const getPlayerByUserId = lobbyOrState => async id => (await getPlayers({ where: { id } })(lobbyOrState))[0];
...
const updateLobbyPlayer = data => lobbyOrState => async (id) => {
    logger.debug(`updateLobbyPlayer ${lobbyOrState.lobby_name} ${id} ${util.inspect(data)}`);
    const [player] = await getPlayers({ where: { id } })(lobbyOrState);
    return player.LobbyPlayer.update(data);
};

const updateLobbyPlayerBySteamId = data => lobbyOrState => async (steamid_64) => {
...
const sortPlayersByCaptainPriority = (playersWithCaptainPriority) => {
    ...
    for (const [player, captain_priority] of playersWithCaptainPriority) {
        if (captain_priority < Number.MAX_SAFE_INTEGER) {
...
const getUserCaptainPriority = (guild, captain_role_regexp, user) => {
    ...
    user.roles.forEach((role) => {
        ...
        if (match) {
            ...
            if (!isNaN(captain_priority) && captain_priority < highest_priority) {
                highest_priority = captain_priority;
            }
        }
        logger.debug(`getUserCaptainPriority ${role.name} ${match} ${highest_priority}`);
    });
    return highest_priority;
};

const playerToCaptainPriority = guild => captain_role_regexp => async player => [player, getUserCaptainPriority(guild, captain_role_regexp, await resolveUser(guild, player))];
...
const getFactionCaptain = lobbyOrState => async faction => (faction > 0 ? getPlayerByUserId(lobbyOrState)([lobbyOrState.captain_1_user_id, lobbyOrState.captain_2_user_id][faction - 1]) : null);
...
const isPlayerDraftable = lobbyOrState => async (player) => {
    if (player.id === lobbyOrState.captain_1_user_id || player.id === lobbyOrState.captain_2_user_id) {
        logger.debug(`isPlayerDraftable INVALID_DRAFT_CAPTAIN ${player.steamid_64} ${player.id === lobbyOrState.captain_1_user_id} ${player.id === lobbyOrState.captain_2_user_id}`);
...
const isCaptain = lobbyOrState => async player => player.id === lobbyOrState.captain_1_user_id || player.id === lobbyOrState.captain_2_user_id;
...
const resetLobbyState = (state) => {
    ...
    case CONSTANTS.STATE_WAITING_FOR_PLAYERS:
        return CONSTANTS.STATE_WAITING_FOR_BOT;
    default:
        return state;
    }
};
...
const killLobby = async (_lobbyState) => {
    ...
    const ready_user_ids = await mapPromise(player => player.id)(getReadyPlayers()(lobbyState));
    const unready_user_ids = await mapPromise(player => player.id)(getNotReadyPlayers()(lobbyState));
...
const loadLobby = async ({
    ...
    try {
        ...
        channel = await findOrCreateChannelInCategory(guild, category, lobby_name);
        role = await getLobbyRole(guild)(lobby_name);
        const lobbyState = {
            guild,
            category,
            channel,
            ...
            captain_1_user_id: lobby.captain_1_user_id,
            captain_2_user_id: lobby.captain_2_user_id,
            match_id: lobby.match_id,
        };
        await addRoleToPlayers(lobbyState);
        logger.debug(`loadLobby done ${lobby_name} ${lobby.state} ${state}`);
