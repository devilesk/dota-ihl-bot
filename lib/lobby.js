...
const calcBalanceTeams = getPlayerRating => (players) => {
    ...
    combs.forEach((comb) => {
        ...
        const weightDiff = Math.abs(weight_1 - weight_2);
        if (weightDiff < bestWeightDiff) {
            bestPairs = [[team_1, team_2]];
            bestWeightDiff = weightDiff;
        }
        else if (weightDiff === bestWeightDiff) {
            bestPairs.push([team_1, team_2]);
        }
    });
    shuffle(bestPairs);
...
const selectCaptainPairFromTiers = captain_rank_threshold => getPlayerRating => (tiers) => {
    ...
    logger.silly(`selectCaptainPairFromTiers captain_rank_threshold: ${captain_rank_threshold} keys: ${keys}`);
...
const getCaptainPriorityFromRoles = (captain_role_regexp, roles) => {
    ...
    logger.silly(`getCaptainPriorityFromRoles ${captain_role_regexp} ${roles} ${priorities}`);
...
const calcDefaultGameMode = default_game_mode => (game_mode_preferences) => {
    ...
    logger.silly(`calcDefaultGameMode ${util.inspect(game_mode_totals)} ${best_count} ${best_game_mode}`);
...
const lobbyToLobbyState = inhouseState => async (lobby) => {
    ...
    try {
        ...
        const players = await getPlayers()(lobbyState);
        logger.silly(`lobbyToLobbyState ${lobby.id}, ${lobbyState.lobby_name}, ${queuers.length}, ${badQueuers.length}, ${players.length}, ${badPlayers.length}, ${lobbyState.captain_1_user_id}, ${lobbyState.captain_2_user_id}`);
...
const getPlayerRatingFunction = (matchmaking_system) => {
    logger.silly(`getPlayerRatingFunction ${matchmaking_system}`);
