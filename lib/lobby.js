...
const cleanMissingPlayers = async lobbyOrState => pipeP(
    getPlayers(),
    filterPromise(negateP(isUserInGuild(lobbyOrState.guild))),
    mapPromise(unvouchUser),
    mapPromise(removePlayer(lobbyOrState)),
)(lobbyOrState);

const cleanMissingQueuers = async lobbyOrState => pipeP(
    getQueuers(),
    filterPromise(negateP(isUserInGuild(lobbyOrState.guild))),
    mapPromise(unvouchUser),
    mapPromise(removeQueuer(lobbyOrState)),
)(lobbyOrState);
...
const checkPlayers = async (lobbyState) => {
    try {
        ...
        try {
            await allPromise(resolveUser(lobbyState.guild));
        }
        catch (e) {
...
const selectCaptainPairFromTiers = captain_rank_threshold => (tiers) => {
    ...
    for (const key of keys) {
        ...
        if (tier.length >= 2) {
            ...
            logger.debug(`selectCaptainPairFromTiers combs ${util.inspect(combs)}`);
            ...
            logger.debug(`selectCaptainPairFromTiers filtered combs ${util.inspect(combs)}`);
            
            if (combs.length) {
                // select random pair
                shuffle(combs);
                logger.debug(`selectCaptainPairFromTiers shuffled combs ${util.inspect(combs)}`);
...
const sortPlayersByCaptainPriority = (playersWithCaptainPriority) => {
    ...
    logger.debug(`sortPlayersByCaptainPriority ${util.inspect(tiers)}`);
    return tiers;
};
...
const isReadyCheckTimedOut = ({ ready_check_timeout, ready_check_time }) => Date.now() - ready_check_time >= ready_check_timeout;
...
const lobbyToLobbyState = ({ findOrCreateChannelInCategory, makeRole }) => ({
    ...
    try {
        ...
        await lock.acquire(`lobby-${lobby.id}`, async () => {
            if (lobby.channel_id) {
                channel = findChannel(guild)(lobby.channel_id);
            }
            if (channel) {
        ...
        await setChannelViewable(guild)(true)(channel);
        await lock.acquire(`lobby-${lobby.id}`, async () => {
            if (lobby.role_id) {
                role = findRole(guild)(lobby.role_id);
            }
            if (role) {
                ...
                role = await makeRole(guild)([])(true)(lobby.lobby_name);
                lobby.role_id = role.id;
                await updateLobbyRole(lobby)(role);
            }
        });
        const inhouseState = {
            guild, category, ready_check_timeout, captain_rank_threshold, captain_role_regexp, default_game_mode, matchmaking_system, leagueid,
        };
        ...
        if (channel) {
            ...
            await setChannelViewable(guild)(false)(channel);
        }
        throw e;
    }
};
...
const LobbyQueueHandlers = {
    ...
    [CONSTANTS.QUEUE_TYPE_CHALLENGE]: () => async (_lobbyState) => {
        ...
        let captain_1 = await getQueuerByUserId(lobbyState)(lobbyState.captain_1_user_id);
        let captain_2 = await getQueuerByUserId(lobbyState)(lobbyState.captain_2_user_id);
...
const getPlayerRatingFunction = (lobbyState) => {
    switch (lobbyState.matchmaking_system) {
    case 'elo':
...
const LobbyStateHandlers = {
    ...
    [CONSTANTS.STATE_BEGIN_READY]: async (_lobbyState) => {
        ...
        lobbyState.channel = await setChannelName(lobbyState.lobby_name)(lobbyState.channel);
        lobbyState.role = await setRoleName(lobbyState.lobby_name)(lobbyState.role);

        // destroy any accepted challenges for players
        await mapPlayers(destroyAllAcceptedChallengeForUser)(lobbyState);

        events.push(CONSTANTS.EVENT_CREATE_LOBBY_QUEUE);
    ...
    [CONSTANTS.STATE_CHECKING_READY]: async (_lobbyState) => {
        ...
        logger.debug(`STATE_CHECKING_READY ${lobbyState.id} ready_check_timeout ${lobbyState.ready_check_timeout} elapsed ${Date.now() - lobbyState.ready_check_time}`);
        if (playersNotReady.length === 0) {
            ...
            for (const player of playersNotReady) {
                ...
                await removeUserFromQueues(player);
                await removePlayer(lobbyState)(player);
            ...
            await returnPlayersToQueue(lobbyState);
            events.push([CONSTANTS.MSG_READY_CHECK_FAILED, playersNotReady]);
            lobbyState.state = CONSTANTS.STATE_WAITING_FOR_QUEUE;
        }
        else {
    ...
    [CONSTANTS.STATE_AUTOBALANCING]: async (_lobbyState) => {
        ...
        if (unassignedCount) {
            ...
            const getPlayerRating = getPlayerRatingFunction(lobbyState);
            await autoBalanceTeams(getPlayerRating)(lobbyState);
    ...
    [CONSTANTS.STATE_COMPLETED]: async _lobbyState => ({ lobbyState: { ..._lobbyState }, events: [] }),
    [CONSTANTS.STATE_FAILED]: async _lobbyState => ({ lobbyState: { ..._lobbyState }, events: [] }),
};

const runLobby = async _lobbyState => lock.acquire(`runLobby-${_lobbyState.id}`, async () => {
    const lobby = await getLobby(_lobbyState);
    let lobbyState = await pipeP(
        createLobbyState(_lobbyState)(_lobbyState),
        validateLobbyPlayers,
    )(lobby);
    logger.debug(`runLobby ${_lobbyState.id} post-validate ${_lobbyState.state} to ${lobbyState.state}`);
    let events;
    ({ lobbyState, events } = await LobbyStateHandlers[lobbyState.state](lobbyState));
    logger.debug(`runLobby ${_lobbyState.id} done. ${lobbyState.state}`);
    await updateLobby(lobbyState);
    return { lobbyState, events };
});
...
const setLobbyTopic = async (lobbyState) => {
    ...
    const queuers = await getActiveQueuers()(lobbyState);
    if (queuers.length) {
        topic += `\n${queuers.length} in queue: ${queuers.map(user => lobbyState.guild.members.get(user.discord_id).displayName).join(', ')}.`;
    }
    const captains = [];
    ...
    if (captains.length) {
        topic += `\nCaptains: ${captains.map(user => lobbyState.guild.members.get(user.discord_id).displayName).join(', ')}.`;
    }
    const readyPlayers = await getReadyPlayers()(lobbyState);
    if (readyPlayers.length) {
        topic += `\n${readyPlayers.length} ready players: ${readyPlayers.map(user => lobbyState.guild.members.get(user.discord_id).displayName).join(', ')}.`;
    }
    const notReadyPlayers = await getNotReadyPlayers()(lobbyState);
    if (notReadyPlayers.length) {
        topic += `\n${notReadyPlayers.length} players not ready: ${notReadyPlayers.map(user => lobbyState.guild.members.get(user.discord_id).displayName).join(', ')}.`;
    }
    const team1 = await getTeam1Players()(lobbyState);
    if (team1.length) {
        topic += `\nTeam 1: ${team1.map(user => lobbyState.guild.members.get(user.discord_id).displayName).join(', ')}.`;
    }
    const team2 = await getTeam2Players()(lobbyState);
    if (team2.length) {
        topic += `\nTeam 1: ${team2.map(user => lobbyState.guild.members.get(user.discord_id).displayName).join(', ')}.`;
    }
