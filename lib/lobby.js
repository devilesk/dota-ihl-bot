...
const getLobby = async lobbyOrState => (lobbyOrState instanceof Sequelize.Model ? lobbyOrState : Db.findLobbyById(lobbyOrState.id));
...
const checkQueueForCaptains = getUserRoles => async lobbyState => Fp.pipeP(
...
const assignCaptains = getUserRoles => async lobbyState => Fp.pipeP(
...
const assignLobbyName = async (lobbyOrState) => {
    ...
    const lobby = await Db.findLobbyByName(lobby_name);
    if (!lobby) {
        await Db.updateLobbyName(lobbyOrState)(lobby_name);
