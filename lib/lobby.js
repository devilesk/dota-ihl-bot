...
const assignLobbyName = async (lobbyOrState) => {
    ...
    const lobby = await findLobby(lobby_name);
    if (!lobby) {
        await updateLobbyName(lobbyOrState.lobby_name)(lobby_name);
        return { ...lobbyOrState, lobby_name };
    }
    return assignLobbyName(lobbyOrState);
}

const renameLobbyChannel = async lobbyState => ({ ...lobbyState, channel: await lobbyState.channel.setName(lobbyState.lobby_name) });

const renameLobbyRole = async lobbyState => ({ ...lobbyState, role: await lobbyState.role.setName(lobbyState.lobby_name) });
...
const LobbyStateHandlers = {
    ...
    [CONSTANTS.STATE_WAITING_FOR_PLAYERS]: async (_lobbyState) => {
        ...
        if (isDotaLobbyReady(lobbyState.dotaBot.factionCache, lobbyState.dotaBot.playerState)) {
            ...
            lobbyState.bot_id = null;
            events.push(CONSTANTS.MSG_LOBBY_STARTED);
...
/*const runLobby = async (_lobbyState, eventEmitter) => {
    ...
    events.forEach(evt => eventEmitter.emit(evt, lobbyState));
    if (lobbyState.state === CONSTANTS.STATE_BOT_STARTED) {
        lobbyState.dotaBot.on(CONSTANTS.MSG_CHAT_MESSAGE, (channel, sender_name, message, chatData) => eventEmitter.emit(CONSTANTS.MSG_CHAT_MESSAGE, lobbyState, channel, sender_name, message, chatData));
        lobbyState.dotaBot.on(CONSTANTS.MSG_LOBBY_PLAYER_JOINED, member => eventEmitter.emit(CONSTANTS.MSG_LOBBY_PLAYER_JOINED, lobbyState, member));
        lobbyState.dotaBot.on(CONSTANTS.MSG_LOBBY_PLAYER_LEFT, member => eventEmitter.emit(CONSTANTS.MSG_LOBBY_PLAYER_LEFT, lobbyState, member));
        lobbyState.dotaBot.on(CONSTANTS.MSG_LOBBY_PLAYER_CHANGED_SLOT, state => eventEmitter.emit(CONSTANTS.MSG_LOBBY_PLAYER_CHANGED_SLOT, lobbyState, state));
...
* @returns {boolean} Whether the message was sent from the lobby state.
*/
const isMessageFromLobby = msg => lobbyState => lobbyState.channel.id === msg.channel.id;
