...
const hasActiveQueuer = lobbyOrState => async user => getLobby(lobbyOrState).then(lobby => (lobby ? lobby.hasActiveQueuer(user) : false));
...
const checkPlayers = async (lobbyState) => {
    try {
        ...
        if (players.length !== 10) {
            return { ...lobbyState, state: CONSTANTS.STATE_FAILED, failReason: `checkPlayers: invalid player count ${players.length}` };
        }
        try {
