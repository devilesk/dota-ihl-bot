...
const lobbyToLobbyState = ({ findOrCreateChannelInCategory, makeRole }) => ({
    ...
    try {
        ...
        const inhouseState = { guild, category, ready_check_timeout, captain_rank_threshold, captain_role_regexp, matchmaking_system };
        
        let lobbyState = await createLobbyState(inhouseState)({ channel, role })(lobby);
...
const createChallengeLobby = async ({ inhouseState, captain_1, captain_2, challenge }) => {        
...
const runLobby = async _lobbyState => {
    return lock.acquire(`runLobby-${_lobbyState.id}`, async () => {
