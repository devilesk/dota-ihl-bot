...
const updatePlayerState = (steamid_64, slot, playerState) => {
    ...
    if (slot == null) {
        ...
        if (team == null) {
            ...
            _playerState[steamid_64] = team;
        }
    }
    logger.silly(`updatePlayerState ${steamid_64} ${slot} ${util.inspect(playerState)} -> ${util.inspect(_playerState)}`);
    return _playerState;
};
...
const isDotaLobbyReady = (teamCache, playerState) => {
    for (const [steamid_64, team] of Object.entries(teamCache)) {
        if (playerState[steamid_64] != team) return false;
    }
    return true;
};
...
const kickPlayer = dotaBot => async user => dotaBot.practiceLobbyKick(parseInt(convertor.to32(user.steamid_64)));

const disconnectDotaBot = async (dotaBot) => {
    logger.silly(`disconnectDotaBot ${dotaBot.steamid_64}`);
    await dotaBot.disconnect();
    await Db.updateBotStatusBySteamId(CONSTANTS.BOT_OFFLINE)(dotaBot.steamid_64);
    return dotaBot;
};

const connectDotaBot = async (dotaBot) => {
    logger.silly(`connectDotaBot ${dotaBot.steamid_64}`);
    await dotaBot.connect();
    await Db.updateBotStatusBySteamId(CONSTANTS.BOT_ONLINE)(dotaBot.steamid_64);
    return dotaBot;
};

const createDotaBotLobby = ({ lobby_name, password, leagueid, game_mode, first_pick, radiant_faction }) => async (dotaBot) => {
    const cm_pick = radiant_faction === first_pick ? Dota2.schema.DOTA_CM_PICK.DOTA_CM_GOOD_GUYS : Dota2.schema.DOTA_CM_PICK.DOTA_CM_BAD_GUYS
    const game_mode_value = Dota2.schema.DOTA_GameMode[game_mode];
    logger.silly(`createDotaBotLobby ${lobby_name} ${password} ${leagueid} ${game_mode} ${game_mode_value} ${cm_pick} ${dotaBot.steamid_64}`);
    let result = await dotaBot.createPracticeLobby({ game_name: lobby_name, pass_key: password, leagueid, game_mode: game_mode_value, cm_pick });
    if (result === steam.EResult.OK) {
        logger.silly('createDotaBotLobby practice lobby created');
        await Db.updateBotStatusBySteamId(CONSTANTS.BOT_IN_LOBBY)(dotaBot.steamid_64);
        logger.silly('createDotaBotLobby bot status updated');
        await dotaBot.practiceLobbyKickFromTeam(dotaBot.accountId);
        await dotaBot.joinLobbyChat();
    }
    else if (result === steam.EResult.Fail) {
        logger.silly('createDotaBotLobby practice lobby failed... retrying');
        await dotaBot.leavePracticeLobby();
        result = await dotaBot.createPracticeLobby({ game_name: lobby_name, pass_key: password, leagueid, game_mode: game_mode_value, cm_pick });
        if (result === steam.EResult.OK) {
            logger.silly('createDotaBotLobby practice lobby created');
            await Db.updateBotStatusBySteamId(CONSTANTS.BOT_IN_LOBBY)(dotaBot.steamid_64);
            logger.silly('createDotaBotLobby bot status updated');
...
const joinDotaBotLobby = ({
    lobby_id, lobby_name, password, leagueid, game_mode, first_pick, radiant_faction,
}) => async (dotaBot) => {
    const cm_pick = radiant_faction === first_pick ? Dota2.schema.DOTA_CM_PICK.DOTA_CM_GOOD_GUYS : Dota2.schema.DOTA_CM_PICK.DOTA_CM_BAD_GUYS
    const game_mode_value = Dota2.schema.DOTA_GameMode[game_mode];
    logger.silly(`joinDotaBotLobby ${lobby_name} ${password} ${leagueid} ${game_mode} ${game_mode_value} ${cm_pick}`);
    const options = { game_name: lobby_name, pass_key: password, leagueid, game_mode: game_mode_value, cm_pick };
...
const loadDotaBotTickets = async (dotaBot) => {
    ...
    const bot = await Db.findBotBySteamId64(dotaBot.steamid_64);
...
class DotaBot extends EventEmitter {
    ...
    constructor(steamClient, steamUser, steamFriends, dotaClient, config) {
        ...
        this.Dota2.on('matchSignedOut', match_id => this.emit(CONSTANTS.EVENT_MATCH_SIGNEDOUT, match_id));
        ...
        this.Dota2.on('chatMessage', (channel, sender_name, message, chatData) => {
            logger.silly(`DotaBot chatMessage ${channel} ${sender_name} {$message} ${util.inspect(chatData)}`);
            if (channel === `Lobby_${this.lobby_id}`) {
    ...
    * */
    ...
    async onSteamClientLoggedOff(eresult) {
        logger.silly('Logged off from Steam. Trying reconnect');
    ...
    async onSteamClientError(error) {
        logger.silly('Connection closed by server. Trying reconnect');
        // Block queue while there's no access to Steam
        this.block();
        await this.connect();
    }
    ...
    onDotaReady() {
        ...
        logger.silly('node-dota2 ready.');
        this.release();
    }

    onDotaUnready() {
        logger.silly('node-dota2 unready.');
        // Block queue when GC is not ready
        this.block();
    }

    onDotaUnhandledMsg(kMsg) {
        logger.silly(`UNHANDLED MESSAGE ${kMsg}`);
    }
     ...
     * */
    async connect() {
        logger.silly('steamClient connecting...');
    ...
    async logOnToSteam() {
        ...
        logger.silly('Set steam persona state and name.');
    }
    ...
    processLobbyUpdate(oldLobby, newLobby) {
        ...
        for (const member of members.left) {
            logger.silly(`processLobbyUpdate member left ${member.id}`);
            this.emit(CONSTANTS.MSG_LOBBY_PLAYER_LEFT, member);
        }

        for (const member of members.joined) {
            logger.silly(`processLobbyUpdate member joined ${member.id}`);
            this.emit(CONSTANTS.MSG_LOBBY_PLAYER_JOINED, member);
        }

        for (const memberState of members.changedSlot) {
            logger.silly(`processLobbyUpdate member slot changed ${util.inspect(memberState.previous)} => ${util.inspect(memberState.current)}`);
            this.emit(CONSTANTS.MSG_LOBBY_PLAYER_CHANGED_SLOT, memberState);
            const steamid_64 = memberState.current.id.toString();
            logger.silly(`processLobbyUpdate steamid_64 ${steamid_64} teamCache ${this.teamCache[steamid_64]} slotToTeam ${slotToTeam(memberState.current.team)}`);
            if (Object.prototype.hasOwnProperty.call(this.teamCache, steamid_64)) {
                if (this.teamCache[steamid_64] !== slotToTeam(memberState.current.team)) {
                    const account_id = parseInt(convertor.to32(steamid_64));
                    logger.silly(`processLobbyUpdate slot change mismatch. kicking ${account_id} ${typeof account_id}`);
                    this.practiceLobbyKickFromTeam(account_id).catch(logger.error);
                }
            }
        }

        if (isDotaLobbyReady(this.teamCache, membersToPlayerState(newLobby.members))) {
            logger.silly('processLobbyUpdate lobby ready');
     ...
     * @async
     * @param {string} steamid_64 - A steam id.
     */
        ...
        return new Promise((resolve, reject) => {
            this.schedule(() => {
                if (this.lobby) {
                    ...
                    logger.silly(`DotaBot inviteToLobby ${steamid_64}`);
                    this.Dota2.inviteToLobby(Long.fromString(steamid_64));
                }
                else {
        ...
        return new Promise((resolve, reject) => {
            this.schedule(() => {
                if (this.lobby) {
                    ...
                    this.Dota2.configPracticeLobby(this.lobby_id, this.lobbyOptions, (err, body) => {
                        if (!err) {
                            resolve(body);
                        }
                        else {
     ...
     * @async
     * @param {number} account_id - An account id.
     */
    async practiceLobbyKickFromTeam(account_id) {
        return new Promise((resolve, reject) => {
            this.schedule(() => {
                if (this.lobby) {
                    logger.silly(`DotaBot practiceLobbyKickFromTeam ${account_id}`);
    ...
    async practiceLobbyKick(account_id) {
        return new Promise((resolve, reject) => {
            this.schedule(() => {
                if (this.lobby) {
                    logger.silly(`DotaBot practiceLobbyKick ${account_id}`);
    ...
    /**
     * Join the lobby by lobby_id.
     * @async
     * @param {string} lobby_id - A lobby id.
     */
    async joinPracticeLobby(lobby_id, options) {
        return new Promise((resolve, reject) => {
            this.schedule(() => {
                this.lobbyOptions = options;
                logger.silly(`DotaBot joinPracticeLobby ${lobby_id}`);
                this.Dota2.joinPracticeLobby(Long.fromString(lobby_id), this.lobbyOptions.pass_key, (err, body) => {
                    if (!err) {
