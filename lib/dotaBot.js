...
const createDotaBotLobby = ({ lobby_name, pass_key }) => async (dotaBot) => {
    logger.debug(`createDotaBotLobby ${lobby_name} ${pass_key} ${dotaBot.steamid_64}`);
    await dotaBot.createPracticeLobby({ game_name: lobby_name, pass_key });
...
const joinDotaBotLobby = ({ lobby_id, lobby_name, pass_key }) => async (dotaBot) => {
    logger.debug(`joinDotaBotLobby ${lobby_name} ${pass_key}`);
    await dotaBot.joinPracticeLobby(lobby_id, { game_name: lobby_name, pass_key });
    await updateBotStatusBySteamId(CONSTANTS.BOT_IN_LOBBY)(dotaBot.steamid_64);
    return dotaBot;
};
...
const startDotaLobby = async (dotaBot) => {
    ...
    try {
        ...
        logger.debug('startDotaLobby bot abandoned current game');
    }
    catch (e) {
        logger.error('', e);
    }
...
class DotaBot extends events.EventEmitter {
    ...
    * */
    ...
    async configPracticeLobby(_options, lobby_id) {
        const options = { ...this.lobbyOptions, ..._options };
        ...
        return new Promise((resolve, reject) => {
            if (lobby_id || this.lobby) {
                this.schedule(() => {
                    ...
                    this.Dota2.configPracticeLobby(lobby_id, options, (err) => {
                        if (err) {
                            reject(err);
                        }
                    });
    ...
    async joinPracticeLobby(lobby_id, _options) {
        const options = { ...defaultLobbyOptions, ..._options };
                ...
                this.Dota2.joinPracticeLobby(lobby_id, options.pass_key, (err, body) => {
                    if (!err) {
                        if (body.result === dota2.schema.lookupEnum('DOTAJoinLobbyResult').values.DOTA_JOIN_RESULT_SUCCESS) {
                            this.practiceLobbyKickFromTeam(this.accountId).then(() => {});
    ...
    async createPracticeLobby(_options) {
        const options = { ...defaultLobbyOptions, ..._options };
        this.lobbyOptions = options;
