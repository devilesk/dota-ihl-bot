...
const createDotaBotLobby = ({ lobby_name, password, leagueid, game_mode, first_pick, radiant_faction }) => async (dotaBot) => {
    const cm_pick = radiant_faction === first_pick ? dota2.schema.lookupEnum('DOTA_CM_PICK').values.DOTA_CM_GOOD_GUYS : dota2.schema.lookupEnum('DOTA_CM_PICK').values.DOTA_CM_BAD_GUYS
    const game_mode_value = dota2.schema.lookupEnum('DOTA_GameMode').values[game_mode];
    logger.debug(`createDotaBotLobby ${lobby_name} ${password} ${leagueid} ${game_mode} ${game_mode_value} ${cm_pick} ${dotaBot.steamid_64}`);
...
const joinDotaBotLobby = ({
    ...
    const cm_pick = radiant_faction === first_pick ? dota2.schema.lookupEnum('DOTA_CM_PICK').values.DOTA_CM_GOOD_GUYS : dota2.schema.lookupEnum('DOTA_CM_PICK').values.DOTA_CM_BAD_GUYS
    const game_mode_value = dota2.schema.lookupEnum('DOTA_GameMode').values[game_mode];
    logger.debug(`joinDotaBotLobby ${lobby_name} ${password} ${leagueid} ${game_mode} ${game_mode_value} ${cm_pick}`);
...
class DotaBot extends events.EventEmitter {
    ...
    constructor(steamClient, steamUser, steamFriends, dotaClient, config) {
        ...
        this.Dota2.on('practiceLobbyUpdate', (lobby) => {
            ...
            if (lobby.match_outcome === dota2.schema.lookupEnum('EMatchOutcome').values.k_EMatchOutcome_RadVictory
                || lobby.match_outcome === dota2.schema.lookupEnum('EMatchOutcome').values.k_EMatchOutcome_DireVictory) {
                this.emit(CONSTANTS.EVENT_MATCH_OUTCOME, lobby.lobby_id, lobby.match_outcome);
            }
        });
    ...
    * */
     ...
     * */
    get lobby_id() {
        return this.Dota2.Lobby ? this.Dota2.Lobby.lobby_id : 0;
    }
    ...
    processLobbyUpdate(oldLobby, newLobby) {
        ...
        for (const memberState of members.changedSlot) {
            ...
            logger.debug(`processLobbyUpdate steamid_64 ${steamid_64} teamCache ${this.teamCache[steamid_64]} slotToTeam ${slotToTeam(memberState.current.team)}`);
            if (Object.prototype.hasOwnProperty.call(this.teamCache, steamid_64)) {
                if (this.teamCache[steamid_64] !== slotToTeam(memberState.current.team)) {
                    ...
                    logger.debug(`processLobbyUpdate slot change mismatch. kicking ${account_id} ${typeof account_id}`);
    ...
    async joinLobbyChat() {
        return new Promise((resolve, reject) => {
            if (this.lobby) {
                resolve(this.joinChat(this.lobbyChannelName, dota2.schema.lookupEnum('DOTAChatChannelType_t').values.DOTAChannelType_Lobby));
            }
            else {
    ...
    async leaveLobbyChat() {
        return new Promise((resolve, reject) => {
            if (this.lobby) {
                resolve(this.leaveChat(this.lobbyChannelName, dota2.schema.lookupEnum('DOTAChatChannelType_t').values.DOTAChannelType_Lobby));
            }
            else {
