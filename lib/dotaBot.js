...
const updatePlayerState = (steamid_64, slot, playerState) => {
    ...
    logger.silly(`DotaBot updatePlayerState ${steamid_64} ${slot} ${util.inspect(playerState)} -> ${util.inspect(_playerState)}`);
    return _playerState;
};
...
const disconnectDotaBot = async (dotaBot) => {
    logger.silly(`DotaBot disconnectDotaBot ${dotaBot.steamid_64}`);
...
const connectDotaBot = async (dotaBot) => {
    logger.silly(`DotaBot connectDotaBot ${dotaBot.steamid_64}`);
...
const createDotaBotLobby = ({
    lobby_name, password, leagueid, game_mode, first_pick, radiant_faction,
}) => async (dotaBot) => {
    ...
    logger.silly(`DotaBot createDotaBotLobby ${lobby_name} ${password} ${leagueid} ${game_mode} ${game_mode_value} ${cm_pick} ${dotaBot.steamid_64}`);
    let result = await dotaBot.createPracticeLobby({
        game_name: lobby_name, pass_key: password, leagueid, game_mode: game_mode_value, cm_pick,
    });
    if (result === steam.EResult.OK) {
        ...
        result = await dotaBot.createPracticeLobby({
            game_name: lobby_name, pass_key: password, leagueid, game_mode: game_mode_value, cm_pick,
        });
        if (result === steam.EResult.OK) {
            logger.silly('DotaBot createDotaBotLobby practice lobby created');
...
const joinDotaBotLobby = ({
    ...
    const cm_pick = radiant_faction === first_pick ? Dota2.schema.DOTA_CM_PICK.DOTA_CM_GOOD_GUYS : Dota2.schema.DOTA_CM_PICK.DOTA_CM_BAD_GUYS;
    const game_mode_value = Dota2.schema.DOTA_GameMode[game_mode];
    logger.silly(`DotaBot joinDotaBotLobby ${lobby_name} ${password} ${leagueid} ${game_mode} ${game_mode_value} ${cm_pick}`);
    const options = {
        game_name: lobby_name, pass_key: password, leagueid, game_mode: game_mode_value, cm_pick,
    };
...
class DotaBot extends EventEmitter {
    ...
    * */
    ...
    async onSteamClientLoggedOff(eresult) {
        logger.silly('DotaBot logged off from Steam. Trying reconnect');
    ...
    processLobbyUpdate(oldLobby, newLobby) {
        ...
        for (const memberState of members.changedSlot) {
            ...
            logger.silly(`DotaBot processLobbyUpdate steamid_64 ${steamid_64} teamCache ${this.teamCache[steamid_64]} slotToTeam ${slotToTeam(memberState.current.team)}`);
            if (Object.prototype.hasOwnProperty.call(this.teamCache, steamid_64)) {
                if (this.teamCache[steamid_64] !== slotToTeam(memberState.current.team)) {
                    ...
                    logger.silly(`DotaBot processLobbyUpdate slot change mismatch. kicking ${account_id} ${typeof account_id}`);
