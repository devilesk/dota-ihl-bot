...
const LobbyStateHandlers = ({
    ...
    async [CONSTANTS.STATE_CHECKING_READY](_lobbyState) {
        ...
        logger.silly(`STATE_CHECKING_READY ${lobbyState.id} ready_check_timeout ${lobbyState.inhouseState.ready_check_timeout} elapsed ${Date.now() - lobbyState.ready_check_time}`);
    ...
    async [CONSTANTS.STATE_ASSIGNING_CAPTAINS](_lobbyState) {
        ...
        if (lobbyState.captain_1_user_id == null || lobbyState.captain_2_user_id == null) {
            ...
            logger.silly(`lobby run assignCaptains captain_1 ${!!captain_1} captain_2 ${!!captain_2}`);
            ...
            logger.silly(`lobby run assignCaptains captains exist ${lobbyState.captain_1_user_id}, ${lobbyState.captain_2_user_id}`);
            lobbyState.state = CONSTANTS.STATE_SELECTION_PRIORITY;
            this.emit(CONSTANTS.MSG_ASSIGNED_CAPTAINS, lobbyState);
        }
        await Db.updateLobby(lobbyState);
    ...
    async [CONSTANTS.STATE_SELECTION_PRIORITY](_lobbyState) {
        ...
        logger.silly(`lobby run STATE_SELECTION_PRIORITY selection_priority ${lobbyState.selection_priority} player_first_pick ${lobbyState.player_first_pick} first pick ${lobbyState.first_pick} radiant_faction ${lobbyState.radiant_faction}`);
        return lobbyState;
    },
    async [CONSTANTS.STATE_DRAFTING_PLAYERS](_lobbyState) {
        ...
        logger.silly(`lobby run STATE_DRAFTING_PLAYERS ${faction} ${captain} ${lobbyState.player_first_pick} ${lobbyState.first_pick} ${lobbyState.radiant_faction}`);
        if (captain) {
    ...
    async [CONSTANTS.STATE_BOT_ASSIGNED](_lobbyState) {
        ...
        if (lobbyState.bot_id != null) {
            ...
            if (dotaBot) {
                ...
                if (!dotaBot.lobby_id || equalsLong(lobbyState.lobby_id, dotaBot.lobby_id)) {
                    ...
                    if (!lobbyState.inhouseState.leagueid || tickets.find(ticket => ticket.leagueid === lobbyState.inhouseState.leagueid)) {
                        ...
                        enterLobbyP.then(async (dotaBot) => {
                            lobbyState.lobby_id = dotaBot.lobby_id.isZero() ? null : dotaBot.lobby_id.toString();
                            ...
                            this.emit(CONSTANTS.MSG_BOT_UNASSIGNED, lobbyState, 'Failed to create lobby.');
                        ...
                        lobbyState = await Lobby.unassignBotFromLobby(lobbyState);
                        this.emit(CONSTANTS.MSG_BOT_UNASSIGNED, lobbyState, `Missing league ticket ${lobbyState.inhouseState.leagueid}.`);
                    }
                }
                else {
                    logger.silly(`lobby_id mismatch. lobbyState.lobby_id ${lobbyState.lobby_id} dotaBot.lobby_id ${dotaBot.lobby_id}`);
                    lobbyState.state = CONSTANTS.STATE_WAITING_FOR_BOT;
                    lobbyState = await Lobby.unassignBotFromLobby(lobbyState);
                    this.emit(CONSTANTS.MSG_BOT_UNASSIGNED, lobbyState, 'In another lobby.');
                }
            }
            await Db.updateLobby(lobbyState);
            return lobbyState;
    ...
    async [CONSTANTS.STATE_BOT_FAILED](_lobbyState) {
        this.emit(CONSTANTS.MSG_BOT_FAILED, _lobbyState);
        return { ..._lobbyState };
    },
