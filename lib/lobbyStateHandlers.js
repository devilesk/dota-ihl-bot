...
const LobbyStateHandlers = ({ DotaBot, Db, Guild, Lobby, MatchTracker, LobbyQueueHandlers }) => ({
    ...
    async [CONSTANTS.STATE_WAITING_FOR_QUEUE](_lobbyState) {
        const lobbyState = LobbyQueueHandlers({ Db, Guild, Lobby })[_lobbyState.queueType](Lobby.checkQueueForCaptains)(_lobbyState);
    ...
    async [CONSTANTS.STATE_BOT_ASSIGNED](_lobbyState) {
        let lobbyState = { ..._lobbyState };
        if (lobbyState.botId != null) {
            const dotaBot = await this.loadBot(lobbyState.botId);
            if (dotaBot) {
                ...
                if (!dotaBot.dotaLobbyId || equalsLong(lobbyState.dotaLobbyId, dotaBot.dotaLobbyId)) {
                    ...
                    if (!lobbyState.inhouseState.leagueid || tickets.find(ticket => ticket.leagueid === lobbyState.inhouseState.leagueid)) {
                        ...
                        enterLobbyP.then(async (_dotaBot) => {
                            lobbyState.dotaLobbyId = _dotaBot.dotaLobbyId.isZero() ? null : _dotaBot.dotaLobbyId.toString();
