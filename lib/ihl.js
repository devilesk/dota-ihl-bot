...
const createInhouseState = async ({ league, guild }) => {
    ...
    logger.silly(`createInhouseState channel created ${league.channel_name} ${channel}`);
    await Guild.setChannelPosition(0)(channel);
    logger.silly('createInhouseState channel position set');
    let adminRole;
    try {
        adminRole = await Guild.makeRole(guild)([])(true)(league.admin_role_name);
    }
    catch (e) {
        logger.error(e);
        if (e instanceof DiscordAPIError && e.message === 'Missing Permissions') {
            throw new Exceptions.MissingDiscordPermission(`Could not create admin role "${league.admin_role_name}".
            Check that bot has "Manage Roles" permission and "${league.admin_role_name}" role does not exist or is below the bot role.`);
        }
        throw e;
    }
