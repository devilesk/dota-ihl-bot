...
*/
const loadLobbiesIntoInhouse = findOrCreateChannelInCategory => getLobbyRole => async (_inhouseState) => {
    ...
    inhouseState.lobbies = await mapPromise(lobby => {
        try {
            return lobbyToLobbyState({ findOrCreateChannelInCategory, getLobbyRole })(inhouseState)(lobby);
        }
        catch (e) {
            return null;
        }
...
*/
const loadQueuesIntoInhouse = async (_inhouseState) => {
    const inhouseState = { ..._inhouseState };
    const queues = await findAllEnabledQueues(inhouseState.guild.id);
    inhouseState.queues = await mapPromise(queue => {
        try {
            const { queueState, lobbyState } = loadQueue({ findOrCreateChannelInCategory, getLobbyRole })(inhouseState)(queue);
            return queueState;
        }
        catch (e) {
            return null;
 ...
 */
const reloadQueueForInhouse = lobbyState => async (inhouseState) => {
    if (lobbyState.queue_type !== CONSTANTS.QUEUE_TYPE_CHALLENGE) {
        const queue = await getQueueFromInhouseByType(inhouseState, lobbyState.queue_type);
        const { queueState } = loadQueue({ findOrCreateChannelInCategory, getLobbyRole })(inhouseState)(queue);
        inhouseState = addQueueToInhouse(inhouseState, queueState);
    }
    else {
        const queue = await getQueueFromInhouseByName(inhouseState, lobbyState.queue_name);
        inhouseState = removeQueueFromInhouse(inhouseState, queueState);
    }
}
