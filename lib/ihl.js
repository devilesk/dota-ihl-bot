...
const parseSteamID64 = async text => {
    ...
    try {
        ...
        else if (text.indexOf('dotabuff.com/players') !== -1 || text.indexOf('opendota.com/players') !== -1 || text.indexOf('stratz.com/en-us/player')) {
        ...
        logger.debug(`parseSteamID64 steamid_64 ${steamid_64}`);
        return steamid_64;
    }
    catch (e) {
...
const registerUser = async (guildId, steamId64, discordId) => {
    ...
    steamId64 = new BigNumber(steamId64).toString();
...
const createInhouseState = async ({
    ...
    let channel = await findOrCreateChannelInCategory(guild, category, channel_name);
