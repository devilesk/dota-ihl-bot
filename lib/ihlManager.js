...
class IHLManager extends EventEmitter {
    ...
    async init(client) {
        ...
        this.matchTracker.on(CONSTANTS.EVENT_MATCH_ENDED, lobby => this.emit(CONSTANTS.EVENT_MATCH_ENDED, lobby));
        this.client = client;
    ...
    */
    ...
    async onSetBotStatus(league, steamid_64, status) {
        await Db.updateBot(league)(steamid_64)({ status });
    }
    ...
    async onLobbyReady(lobbyState) {
        ...
        this.emit(CONSTANTS.EVENT_RUN_LOBBY, lobbyState, [CONSTANTS.STATE_WAITING_FOR_PLAYERS]);
    }
    ...
    async onMatchEnd(lobby) {
        ...
        this.emit(CONSTANTS.MSG_MATCH_ENDED, lobbyState, inhouseState);
    ...
    getBot(lobbyState) {
        return lobbyState.bot_id != null ? this.bots[lobbyState.bot_id] : null;
    }
    ...
    attachEventHandlers() {
        ...
        this.on(CONSTANTS.EVENT_BOT_SET_STATUS, (league, steamid_64, status) => this.queueEvent(this.onSetBotStatus, [league, steamid_64, status]));
        ...
        this.on(CONSTANTS.EVENT_MATCH_ENDED, lobby => this.queueEvent(this.onMatchEnd, [lobby]));

        this.on(CONSTANTS.EVENT_LOBBY_SET_FP, async (lobbyState, cm_pick) => {
            const dotaBot = this.getBot(lobbyState);
            if (dotaBot) {
    ...
    attachMessageHandlers() {
        ...
        this.on(CONSTANTS.MSG_MATCH_ENDED, async (lobbyState, inhouseState) => sendMatchEndMessage(inhouseState)(lobbyState));
