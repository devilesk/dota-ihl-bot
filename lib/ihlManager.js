...
const loadInhouseState = eventEmitter => guilds => async league => pipeP(
    ...
    loadQueuesIntoInhouse(eventEmitter)({ findOrCreateChannelInCategory, getLobbyRole }),
    loadLobbiesIntoInhouse(eventEmitter)({ findOrCreateChannelInCategory, getLobbyRole }),
...
const getLobbyByLobbyName = (inhouseStates, lobby_name) => {
    ...
    for (const inhouseState of inhouseStates) {
        const lobbyState = getLobbyFromInhouse(inhouseState, lobby_name);
        if (lobbyState) {
...
const isMessageFromLobby = (inhouseStates, msg) => {
    return getLobbyByChannelId(inhouseStates, msg.channel.guild.id, msg.channel.id) != null;
};
...
class IHLManager extends events.EventEmitter {
    ...
    async createChallengeLobbyForInhouse(_inhouseState, challenge, giver, receiver) {
        const inhouseState = await createChallengeLobbyForInhouse({ resolveUser, findOrCreateChannelInCategory, getLobbyRole })(_inhouseState, challenge, this, giver, receiver);
        this.inhouseStates = addInhouseState(this.inhouseStates, inhouseState);
    }
    ...
    async banInhouseQueue(inhouseState, user, timeout) {
        await banInhouseQueue(inhouseState, user, timeout);
        await leaveAllQueues(inhouseState, user, this);
        await this.checkInhouseQueues(inhouseState);
    }
    ...
    async reloadQueue(lobbyState) {
        ...
        inhouseState = await reloadQueueForInhouse(inhouseState, lobbyState);
        this.inhouseStates = addInhouseState(this.inhouseStates, inhouseState);
    }
    ...
    attachMessageHandlers() {
        ...
        this.on(CONSTANTS.EVENT_INHOUSE_CREATED, async (inhouseState) => inhouseState.channel.send('Inhouse league started.').catch(console.error));
        this.on(CONSTANTS.EVENT_QUEUES_LOADED, async (inhouseState, total, loaded) => inhouseState.channel.send(`${loaded}/${total} queues loaded.`).catch(console.error));
        this.on(CONSTANTS.EVENT_LOBBIES_LOADED, async (inhouseState, total, loaded) => inhouseState.channel.send(`${loaded}/${total} lobbies loaded.`).catch(console.error));
        
        this.on(CONSTANTS.EVENT_DISPLAY_LOBBY_STATUS, async (lobbyState) => {
