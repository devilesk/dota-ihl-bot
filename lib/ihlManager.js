...
const {
    createInhouseState, loadLobbiesIntoInhouse, runLobbiesForInhouse, addLobbyToInhouse, checkInhouseQueue, createNewLobbyForInhouse, joinInhouseQueue, leaveInhouseQueue, banInhouseQueue,
} = require('./ihl');
const {
    pipeP, mapPromise,
} = require('./util/fp');
...
* @function 
* @param {Guild} guild - A list of guilds to initialize leagues with.
* @param {string|GuildMember} member - A name or search string for an inhouse player or their guild member instance.
...
* @function 
* @param {Guild[]} guilds - A list of guilds to initialize leagues with.
* @param {League} league - A database league record.
* @returns {Object} An object containing the league and its guild.
*/
...
* @param {EventEmitter} eventEmitter - The event listener object.
* @param {Guild[]} guilds - A list of guilds to initialize leagues with.
* @param {League[]} leagues - A list of database league records.
...
const getLobbyByChannelId = (inhouseStates, guildId, channelId) => {
    ...
    return _inhouseState ? [_inhouseState.lobbies.find(lobbyState => lobbyState.channel.id === channelId), _inhouseState] : null;
};
...
* @function
* @param {module:ihl.InhouseState[]} inhouseStates - A list of inhouse states.
* @param {Message} msg - A discord message sent from a lobby channel.
...
class IHLManager extends events.EventEmitter {
     ...
     * @async
     * @param {Guild} guild - A discord.js guild.
     * @param {User} user - A discord.js user.
     */
     ...
     * @param {module:lobby.LobbyState} _lobbyState - An inhouse lobby state.
     */
    registerLobbyTimeout(_lobbyState) {
        this.unregisterLobbyTimeout(_lobbyState);
        const delay = Math.max(0, _lobbyState.ready_check_time + _lobbyState.ready_check_timeout - Date.now());
        logger.debug(`ihlManager registerLobbyTimeout ${_lobbyState.ready_check_time} ${_lobbyState.ready_check_timeout}. timeout ${delay}ms`);
        this.lobbyTimeoutTimers[_lobbyState.lobby_name] = setTimeout(() => {
            logger.debug(`ihlManager lobby ${_lobbyState.lobby_name} timed out`);
            this.queueEvent(this.onLobbyTimedOut, [_lobbyState]);
        }, delay);
    }
     ...
     * @param {module:lobby.LobbyState} _lobbyState - An inhouse lobby state.
     */
    unregisterLobbyTimeout(_lobbyState) {
        logger.debug('ihlManager unregisterLobbyTimeout');
        if (this.lobbyTimeoutTimers[_lobbyState.lobby_name]) {
            clearTimeout(this.lobbyTimeoutTimers[_lobbyState.lobby_name]);
        }
        this.lobbyTimeoutTimers[_lobbyState.lobby_name] = null;
    }
     ...
     * @param {module:lobby.LobbyState} _lobbyState - An inhouse lobby state.
     */
    async onLobbyTimedOut(_lobbyState) {
        logger.debug('ihlManager onLobbyTimedOut');
        this.lobbyTimeoutTimers[_lobbyState.lobby_name] = null;
        await this.runLobby(_lobbyState, [CONSTANTS.STATE_CHECKING_READY]);
    }
     ...
     * @param {module:lobby.LobbyState} _lobbyState - An inhouse lobby state.
     */
    async onPlayersReady(_lobbyState) {
     ...
     * @async
     * @param {module:lobby.LobbyState} _lobbyState - An inhouse lobby state.
     * @param {User} user - An inhouse user.
     */
    async onPlayerReady(_lobbyState, user) {
        logger.debug('ihlManager onPlayerReady');
        await setPlayerReady(true)(_lobbyState)(user.steamid_64);
        await this.runLobby(_lobbyState, [CONSTANTS.STATE_CHECKING_READY]);
    }
     ...
     * @async
     * @param {Message} msg - A discord.js message.
     * @param {GuildMember} member - A discord.js guild member.
     */
     ...
     * @param {module:lobby.LobbyState} _lobbyState - An inhouse lobby state.
     */
    async onMatchEnd(_lobbyState) {
        logger.debug('ihlManager onMatchEnd');
        await this.runLobby(_lobbyState, [CONSTANTS.STATE_MATCH_IN_PROGRESS]);
    }
    ...
    attachMessageHandlers() {
        ...
        this.on(CONSTANTS.EVENT_LOBBY_KILLED, async (lobbyState) => {
            const [, inhouseState] = getLobbyByLobbyName(this.inhouseStates, lobbyState.lobby_name);
            return inhouseState.channel.send('Lobby killed.').catch(console.error);
        });
