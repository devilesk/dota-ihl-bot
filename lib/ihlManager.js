...
const createNewLeague = ({ eventEmitter, inhouseStates }) => async (guild) => pipeP(
    initLeague,
    league => ({ league, guild }),
    createInhouseState(eventEmitter),
    addInhouseState(inhouseStates),
)(guild);

const createChallengeLobby = ({ eventEmitter, inhouseStates }) => async ({ inhouseState, challenge, captain_1, captain_2 }) => pipeP(
    createChallengeLobbyForInhouse({ resolveUser, findOrCreateChannelInCategory, makeRole }),
    addInhouseState(inhouseStates),
)({ eventEmitter, inhouseState, challenge, captain_1, captain_2 });

const runLobbies = ({ eventEmitter, inhouseStates }) => async (inhouseState) => pipeP(
...
class IHLManager {
    ...
    async init(client) {
        ...
        this.inhouseStates = await loadInhouseStatesFromLeagues(this.eventEmitter)(this.client.guilds);
    ...
    async createNewLeague(guild) {
        this.inhouseStates = await createNewLeague(this)(guild);
    }

    /**
     * Creates a new challenge lobby.
     * @async
     * @param {module:ihl.InhouseState} inhouseState - The inhouse for the challenge.
     * @param {module:db.Challenge} challenge - The challenge object.
     * @param {module:db.User} captain_1 - The challenge giver.
     * @param {module:db.User} captain_2 - The challenge receiver.
     */
    async createChallengeLobby(inhouseState, challenge, captain_1, captain_2) {
        this.inhouseStates = await createChallengeLobby(this)({ inhouseState, challenge, captain_1, captain_2 });
    }

    /**
     * Checks the inhouse queues.
     * @async
     * @param {module:ihl.InhouseState} inhouseState - The inhouse to check.
     */
    async runLobbies(inhouseState) {
        this.inhouseStates = await runLobbies(this)(inhouseState);
    }
    ...
    async joinAllQueues(inhouseState, user) {
        await joinAllQueues(inhouseState, user, this.eventEmitter);
        await this.runLobbies(inhouseState);
    }
    ...
    async joinLobbyQueue(lobbyState, user) {
        await joinLobbyQueue(user, this.eventEmitter)(lobbyState);
    ...
    async leaveAllQueues(inhouseState, user) {
        await leaveAllQueues(inhouseState, user, this.eventEmitter);
        await this.runLobbies(inhouseState);
    }
    ...
    async leaveLobbyQueue(lobbyState, user) {
        await leaveLobbyQueue(user, this.eventEmitter)(lobbyState);
        await this.runLobby(lobbyState, [CONSTANTS.STATE_WAITING_FOR_QUEUE]);
    }
        ...
        if (!inhouseState) {
            ...
            if (!lobbyState) {
                if (!states.length || states.indexOf(lobbyState.state) !== -1) {
                    ...
                    lobbyState = await runLobby(lobbyState, this.eventEmitter);
                    if (lobbyState.state === CONSTANTS.STATE_KILLED) {
                        ...
                        this.eventEmitter.emit(CONSTANTS.MSG_LOBBY_KILLED, inhouseState);
                    }
                    else {
    ...
    async reloadQueue(lobbyState) {
        ...
        inhouseState = await reloadQueueForInhouse(this.eventEmitter)({ findOrCreateChannelInCategory, makeRole })(lobbyState)(inhouseState);
        this.inhouseStates = addInhouseState(this.inhouseStates)(inhouseState);
    }
    ...
    attachEventHandlers() {
        ...
        this.eventEmitter.on(CONSTANTS.EVENT_DISCORD_MESSAGE, (msg) => {
    ...
    attachMessageHandlers() {
        ...
        this.eventEmitter.on(CONSTANTS.MSG_MATCH_ENDED, async (lobbyState) => sendMatchEndMessage(this.inhouseStates, lobbyState));
