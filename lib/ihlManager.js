...
const findUser = guild => async (member) => {
    ...
    logger.silly(`findUser ${util.inspect(member)} ${discord_id_matches}`);
    if (discord_id_matches) {
        ...
        // check exact discord nickname match
        discord_user = guild.members.find(guildMember => (guildMember.nickname || '').toLowerCase() === member.toLowerCase());
        if (discord_user) {
            logger.silly(`findUser matched on nickname exact ${member.toLowerCase()}`);
            ...
            if (discord_user) {
                ...
                if (steamid_64 != null) {
                    logger.silly(`findUser matched on steamid_64 ${steamid_64}`);
    ...
    if (user && discord_user) {
        logger.silly(`findUser ${user.id} ${discord_user.id} ${result_type}`);
...
class IHLManager extends EventEmitter {
    ...
    get inviteUrl() {
        return `https://discordapp.com/oauth2/authorize/?permissions=${this.botPermissions}&scope=bot&client_id=${process.env.CLIENT_ID}`
    }
    ...
    async onDiscordMessage(msg) {
        if (msg.author.id !== this.client.user.id) {
            logger.silly(`ihlManager onDiscordMessage ${msg.channel.id} ${msg.author.id} ${msg.content}`);
    ...
    */
    ...
    getBot(bot_id) {
        logger.silly(`ihlManager getBot ${bot_id}`);
        return bot_id != null ? this.bots[bot_id] : null;
    }

    async loadBot(bot_id) {
        logger.silly(`ihlManager loadBot ${bot_id}`);
    ...
    async removeBot(bot_id) {
        logger.silly(`ihlManager removeBot ${bot_id}`);
