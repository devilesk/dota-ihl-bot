...
class IHLManager {
    ...
    async createNewLeague(guild) {
        this.inhouseStates = await createNewLeague(this.eventEmitter)(this.inhouseStates)(guild);
    }
    ...
    async createChallengeLobbyForInhouse(_inhouseState, challenge, giver, receiver) {
        const inhouseState = await createChallengeLobbyForInhouse({ resolveUser, findOrCreateChannelInCategory, makeRole })({ inhouseState: _inhouseState, challenge, eventEmitter: this.eventEmitter, captain_1: giver, captain_2: receiver });
