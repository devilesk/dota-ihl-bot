    ...
    */
    async onDiscordMessage(msg) {
        if (msg.author.id !== this.client.user.id) {
            logger.silly(`ihlManager onDiscordMessage ${msg.channel.guild} ${msg.channel.name} ${msg.channel.id} ${msg.author.id} ${msg.content}`);
    ...
    */
    ...
    async onCreateLobbyQueue(_lobbyState) {
        ...
        if (queue && queue.queueType !== CONSTANTS.QUEUE_TYPE_CHALLENGE) {
            ...
            this[CONSTANTS.EVENT_RUN_LOBBY](lobbyState, [CONSTANTS.STATE_NEW]).catch(e => logger.error(e))
                .then(() => this.emit('lobby-queue-created')); // test hook event
        }
    }
    ...
    async onLobbyReady(dotaLobbyId) {
        ...
        logger.silly(`ihlManager onLobbyReady ${lobbyState.id} ${lobbyState.lobbyName}`);
        this[CONSTANTS.EVENT_RUN_LOBBY](lobbyState, [CONSTANTS.STATE_WAITING_FOR_PLAYERS]).catch(e => logger.error(e));
    }
    ...
    async botLeaveLobby(lobbyState) {
        logger.silly(`ihlManager botLeaveLobby ${lobbyState.id} ${lobbyState.botId}`);
    ...
    async validateLobbyStateBot(lobbyState) {
        if (lobbyState.botId && DotaBot.validBotLobbyStates.indexOf(lobbyState.state) === -1) {
            logger.silly(`validateLobbyStateBot invalid lobby state for bot detected. Bot ${lobbyState.botId} leaving lobbyState ${lobbyState.id} ${lobbyState.state}.`);
            await this.botLeaveLobby(lobbyState);
