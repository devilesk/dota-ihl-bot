...
class IHLManager {
    ...
    */
    async runLobbiesForInhouse(inhouseState) {
        ...
        for (const lobby of lobbies) {
            const lobbyState = await pipeP(
                Lobby.lobbyToLobbyState({ findOrCreateChannelInCategory, makeRole })(inhouseState),
    ...
    async runLobby(_lobbyState, states = []) {
        ...
        logger.debug(`ihlManager runLobby ${_lobbyState.id} ${_lobbyState.state} ${states} ${!states.length || states.indexOf(lobby.state) !== -1}`);
    ...
    async onCreateLobbyQueue(_lobbyState) {
        ...
        if (queue && queue.queue_type !== CONSTANTS.QUEUE_TYPE_CHALLENGE) {
            ...
            const lobbyState = await createLobby({ findOrCreateChannelInCategory, makeRole })(_lobbyState.inhouseState)(queue);
    ...
    async onSetLobbyState(lobbyState, state) {
        ...
        await updateLobbyState(lobbyState)(state);
    ...
    async onMatchEnd(lobby) {
        ...
        const lobbyState = await Lobby.lobbyToLobbyState({ findOrCreateChannelInCategory, makeRole })(inhouseState)(lobby);
    ...
    attachMessageHandlers() {
        ...
        this.eventEmitter.on(CONSTANTS.MSG_ASSIGNED_CAPTAINS, async (lobbyState) => {
            ...
            const captain_member_1 = await resolveUser(lobbyState.inhouseState.guild)(captain_1.discord_id);
            const captain_member_2 = await resolveUser(lobbyState.inhouseState.guild)(captain_2.discord_id);
...
Object.assign(IHLManager.prototype, LobbyStateHandlers);
