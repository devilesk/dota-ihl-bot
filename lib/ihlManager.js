...
class IHLManager extends events.EventEmitter {
    ...
    async checkInhouseQueue(guild) {
        ...
        if (inhouseState) {
            try {
                ...
                inhouseState.channel.send('Queue pop error. Resetting queue.').catch(console.error);
            }
    ...
    /**
     * Adds a user to the inhouse queue and performs a queue pop check.
     * @async
     * @param {external:Guild} guild - A discord.js guild.
    ...
    async joinInhouseQueue(guild, user) {
        ...
        if (inhouseState) {
            ...
            await joinInhouseQueue(inhouseState, user, this);
            await this.checkInhouseQueue(guild);
        }
        else {
    ...
    async leaveInhouseQueue(guild, user) {
        ...
        if (inhouseState) {
            ...
            await leaveInhouseQueue(inhouseState, user, this);
        }
        else {
    ...
    attachMessageHandlers() {
        ...
        this.on(CONSTANTS.EVENT_ASSIGNED_CAPTAINS, async (lobbyState) => {
            const captain_user_1 = await resolveUser(lobbyState.guild, { steamid_64: lobbyState.captain_1 });
            const captain_user_2 = await resolveUser(lobbyState.guild, { steamid_64: lobbyState.captain_2 });
