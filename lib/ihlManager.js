...
const getLobbyByChannelId = (inhouseStates, guildId, channelId) => {
    ...
    const lobbyState = _inhouseState ? _inhouseState.lobbies.find(lobbyState => lobbyState.channel.id === channelId) || null : null;
...
const getLobbyByLobbyName = (inhouseStates, guildId, lobby_name) => {
    const _inhouseState = getInhouseState(inhouseStates, guildId) || null;
    const lobbyState = _inhouseState ? _inhouseState.lobbies.find(lobbyState => lobbyState.lobby_name === lobby_name) || null : null;
    return [lobbyState, _inhouseState];
};
