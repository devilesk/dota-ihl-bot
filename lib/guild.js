...
const setChannelName = name => async (_channel) => {
    ...
    const channel = await _channel.setName(name);
    logger.silly(`renamed channel id ${_channel.id} ${oldName} to {channel.name}`);
    return channel;
}
...
//const setChannelPosition = position => async channel => channel.setPosition(position).catch(e => logger.error(e));
