...
const findAllActiveLobbies = async guild_id => db.Lobby.findAll({ where: { state: { [Op.and]: { [Op.ne]: CONSTANTS.STATE_MATCH_ENDED, [Op.ne]: CONSTANTS.STATE_KILLED } } }, include: [{ model: db.League, where: { guild_id: guild_id } }] });
...
const findOrCreateMatch = async lobbyState => db.Match.findOrCreate({
    where: {
        match_id: lobbyState.match_id,
        lobby_id: lobbyState.id,
    },
    defaults: {
        match_id: lobbyState.match_id,
        lobby_id: lobbyState.id,
