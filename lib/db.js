...
const findAllActiveLobbiesForInhouse = async guild_id => db.Lobby.findAll({ where: { state: { [Op.notIn]: [CONSTANTS.STATE_COMPLETED, CONSTANTS.STATE_KILLED, CONSTANTS.STATE_FAILED] } }, include: [{ model: db.League, where: { guild_id: guild_id } }] });
...
const findAllLobbiesInStateForInhouse = state => async guild_id => db.Lobby.findAll({ where: { state }, include: [{ model: db.League, where: { guild_id: guild_id } }] });
...
const findLobbyByName = async lobby_name => db.Lobby.scope({ method: ['lobby_name', lobby_name] }).find();

const findLobbyById = async id => db.Lobby.findOne({ where: { id } });

const findLobbyByDiscordChannel = guild_id => async channel_id => db.Lobby.findOne({ where: { guild_id, channel_id } });
