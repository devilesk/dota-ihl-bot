...
const findAllUnassignedBot = async () => db.Bot.findAll({
    include: [{
        ...
        attributes: [], // do not return any columns of the Lobby table
    }],
    where: { '$Lobbies.bot_id$': { [Op.eq]: null } },
});

const findAllUnassignedBotForLeague = async league => db.Bot.findAll({
    where: {
        '$Lobbies.bot_id$': { [Op.eq]: null },
...
const findTicketByDotaLeagueId = async id => db.Ticket.findOne({ where: { leagueid } });
