...
const findOrCreateLeague = guild_id => async queues => db.sequelize.transaction(async (t) => {
    ...
    for (const queue of queues) {
        await db.Queue.findOrCreate({
            where: { league_id: league.id, queue_type: queue.queue_type },
            defaults: { queue_name: queue.queue_name },
            include: [db.League],
            transaction: t,
        });
    }
...
const findOrCreateQueue = async (league, enabled, queue_type, queue_name) => db.Queue.findOrCreate({
    where: {
        league_id: league.id, enabled, queue_type,
    },
    defaults: { enabled, queue_name },
    include: [{
        model: db.League,
        where: {
            id: league.id,
