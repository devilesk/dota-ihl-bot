...
const findAllActiveLobbies = async guild_id => db.Lobby.findAll({ where: { state: { [Op.notIn]: [CONSTANTS.STATE_MATCH_ENDED, CONSTANTS.STATE_KILLED] } }, include: [{ model: db.League, where: { guild_id: guild_id } }] });

const findActiveLobbiesForUser = async user => user.getLobbies({ where: { state: { [Op.notIn]: [CONSTANTS.STATE_MATCH_ENDED, CONSTANTS.STATE_KILLED] } } });
