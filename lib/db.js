...
const findOrCreateBot = async (league, steamid_64, account_name, persona_name, password) => db.Bot.findOrCreate({
    where: { league_id: league.id, steamid_64 },
...
const findAllBots = async league => db.Bot.findAll({ where: { league_id: league.id } });
...
const updateBot = league => steamid_64 => async values => db.Bot.update(values, { where: { league_id: league.id, steamid_64 } });
...
const destroyBotBySteamID64 = league => async steamid_64 => db.Bot.destroy({ where: { league_id: league.id, steamid_64 } });
