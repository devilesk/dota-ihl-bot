...
const updatePlayerRatings = async lobbyState => {
    ...
    const team1 = await getTeam1Players()(lobby);
    const team2 = await getTeam2Players()(lobby);
    ...
    for (const player of team1) {
        await updateUserRating(player)(player.rating + D1);
    }
    for (const player of team2) {
        await updateUserRating(player)(player.rating + D2);
    }
}
...
const setMatchPlayerDetails = async _lobby => {
    ...
    for (const player_data of lobby.odota_data.players) {
        ...
        const player = players.find(player => player.steamid_64 === steamid_64);
        const data = {
            win: player_data.win,
    ...
    await updateLobbyWinner(lobby)(winner);
}
...
const createMatchEndMessageEmbed = async match_id => {
    ...
    const captain_1_data = odota_data.players.find(player => player.account_id == convertor.to32(captain_1.steamid_64));
    const captain_2_data = odota_data.players.find(player => player.account_id == convertor.to32(captain_2.steamid_64));
