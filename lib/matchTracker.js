...
const setMatchDetails = async (lobbyState) => {
    ...
    const [match] = await findOrCreateMatch(lobby);
    const data = {};
    if (!match.odata_data) {
    ...
    logger.debug(`matchTracker setMatchDetails ${util.inspect(match)}`);
    return match;
};

const createMatchEndMessageEmbed = async (match_id) => {
    const match = await findMatch(match_id);
    const valve_data = match.valve_data.result;
    const odota_data = match.odota_data;
    const lobby = await match.getLobby();
        ...
        while (this.started) {
            ...
            await Promise.mapSeries(
                ...
                async (lobbyState) => {
                    ...
                    const match = await setMatchDetails(_lobbyState);
                    if (match.odota_data && match.valve_data) {
