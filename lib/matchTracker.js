...
const updatePlayerRatings = async (lobbyState) => {
    ...
    for (const player of faction1) {
        await Lobby.updateLobbyPlayer({ ratingDiff: D1 })(lobby)(player.id);
    ...
    for (const player of faction2) {
        await Lobby.updateLobbyPlayer({ ratingDiff: D2 })(lobby)(player.id);
    ...
    for (const playerData of lobby.odotaData.players) {
        if (playerData.account_id) {
            ...
            if (player) {
                ...
                await Lobby.updateLobbyPlayerBySteamId(data)(_lobby)(steamId64);
