...
const {
    findAllInProgressLobbies, findLobbyByMatchId,
} = require('./db');
const {
    getPlayers, getLobby, updateLobbyPlayerBySteamId,
} = require('./lobby');
...
const setMatchPlayerDetails = async (lobbyState) => {
    const lobby = await getLobby(lobbyState);
    ...
    for (let player_data of lobby.odota_data.players) {
        ...
        await updateLobbyPlayerBySteamId(data)(lobbyState)(steamid_64);
    }
}
            ...
            await Promise.mapSeries(
                ...
                async (lobbyState) => {
                    ...
                    if (lobby.odota_data && lobby.valve_data) {
                        await setMatchPlayerDetails(_lobbyState);
                        this.eventEmitter.emit(CONSTANTS.EVENT_MATCH_ENDED, _lobbyState);
                        this.eventEmitter.emit(CONSTANTS.MSG_MATCH_ENDED, _lobbyState);
                    }
                },
            )
                .delay(3000);
